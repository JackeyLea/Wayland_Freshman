...
create_window()
{
	...
	if(buffer==NULL){
		printf("Cannot create new buffer.\n");
		exit(-1);
	}
...
static void handle_configure(void *data, struct zxdg_surface_v6 *surface, uint32_t serial) {
						...
						uint32_t id, const char *interface, uint32_t version)
{
	...
	{
		BIND_WL_REG(registry, xdg_shell, id, &zxdg_shell_v6_interface, 1);
	}
